...
export class CompanyAccountComponent implements OnInit {
  ...
  @Output() accountListEdited = new EventEmitter<Account[]>();
  page = 0;
  size = 10;
...
if(this.searchQuery!=null || this.searchQuery!="" ){
  ...
  onCodeSearch(event: any) {
    this.subscriptions.add(this.accountService.find('company.id:' + this.selectedCompany.id +',code~' + event.query).subscribe(
      data => this.codeaccountList = data.map(f => f.code)
    ));
  }
  onNameSearch(event: any) {
    this.subscriptions.add(this.accountService.find('company.id:' + this.selectedCompany.id +',name~' + event.query).subscribe(
