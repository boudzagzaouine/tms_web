import { TermInsurance } from './../../models/term-insurance';
...
@Injectable()
export class TermInsuranceTermService {
    controller = 'TermInsurances';

    private TermInsuranceList: TermInsurance[] = [];

    TermInsuranceListChanged = new Subject<TermInsurance[]>();
    ...
    findAll(): Observable<TermInsurance[]> {
    ...
    findById(id: number): Observable<TermInsurance> {
    ...
    set(TermInsurance: TermInsurance): TermInsurance {
        this.proxy.set(this.controller, TermInsurance).subscribe(
            data => {
                this.emitChanges();
    ...
    add(TermInsurance: TermInsurance): TermInsurance {
        this.proxy.add(this.controller, TermInsurance).subscribe(
            data => {
                this.emitChanges();
                this.toastr.success('Item was saved successfully', 'Save');
                return data;
    ...
    delete(TermInsurance: TermInsurance) {
        this.proxy.delete(this.controller, TermInsurance.id).subscribe(
            data => {
                this.emitChanges();
                  this.toastr.success(
