...
export class DataRecoveryComponent implements OnInit {
    ...
    models : ImportClass[]=[];
    model: ImportClass= new ImportClass();
    dataString : string;
    idClass : number=0 ;
    jsonData : any[]=[];
  constructor(private addressDeliveryService: AddressDeliveryService,
  ...
  ngOnInit() {
    ...
    deliveryAddress.name="adresse de livraison"; this.models.push(deliveryAddress);
...
// }
  ...
  if(jsonData[0]==null){
    ...
    this.spinner.show();
    this.addressDeliveryService.addDataExchangeAddressDelivery(jsonData).subscribe(
      data =>{
    console.log(data);
