import { Injectable } from '@angular/core';
import {PurchaseOrder} from '../../models';
import {ProxyService} from './proxy.service';
import {Observable} from 'rxjs/Observable';
import { Subject } from 'rxjs/Subject';
import { ToastrService } from 'ngx-toastr';

@Injectable()
export class PurchaseOrderService {

  controller = 'purshaseOrders';
  purchaseOrderListChanged = new Subject<PurchaseOrder[]>();
  constructor(private proxy: ProxyService,
            private toastr: ToastrService) {
  }

  private emitChanges() {
      this.findAll().subscribe(
          data => this.purchaseOrderListChanged.next(data)
      );
  }

  findAll(): Observable<PurchaseOrder[]> {
    console.log('from purchaseOrder service findAll');
    return this.proxy.findAll(this.controller);
  }

  find(search: string): Observable<PurchaseOrder[]> {
    return this.proxy.find(this.controller, search);
  }

  findById(id: number): Observable<PurchaseOrder> {
    // let TOKEN = this.token.computeToken('ems@ems.com', 'EMS', '77d2896c3eb544541f9389fe42651b0d');
    return this.proxy.findById(this.controller, id);
  }

  size() {
    return this.proxy.size(this.controller);
  }

  findAllPagination(page: number, size: number) {
    return this.proxy.findAllPagination(this.controller, page, size);
  }

  findPagination(page: number, size: number, search: string) {
    return this.proxy.findPagination(this.controller, search, page, size);
  }
  sizeSearch(search: string) {
    return this.proxy.sizeSearch(this.controller, search);
  }
  set(purchaseOrder: PurchaseOrder) {
    this.proxy.set(this.controller, purchaseOrder).subscribe(
        (data: PurchaseOrder) => {
            this.emitChanges();
            this.toastr.success('L\'élément est enrgistré avec succès', 'Edition');
        },
        error =>
            this.toastr.error(
                'L\'élément n\'est pas enregistré ',
                'Edition'
            )
    );
  }
  add(purchaseOrder: PurchaseOrder) {
    this.proxy.add(this.controller, purchaseOrder).subscribe(
        (data: PurchaseOrder) => {
            this.emitChanges();
            this.toastr.success('Elément est enrgistré avec succès', 'Edition');
        },
        error =>
            this.toastr.error(
                'Elément n\'est pas enregistré ',
                'Edition'
            )
    );
  }

  delete(user: PurchaseOrder) {
    this.proxy.delete(this.controller, user.id).subscribe(
        data => {
            this.toastr.success(
                'Elément supprimé avec succès',
                'Suppression'
            );
            this.emitChanges();
        },
        error => this.toastr.error('Erreur de suppression', 'Suppression')
    );
}

  generateCode(): string {
    return 'SO' + (Date.now() / 100000000).toFixed();
  }

}
