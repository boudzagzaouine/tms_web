...
export class TransportEditComponent implements OnInit {
  ...
  constructor(private transportService: TransportServcie,
  ...
  initForm() {
    this.transportForm = new FormGroup({
      ...
      'line1': new FormControl(this.selectAddress.line1, Validators.required),
  ...
  onSubmit() {
    ...
    this.selectedtransport.description = this.transportForm.value['description'];
    ...
    this.addressService.set(this.selectAddress).subscribe(
      dataA => {
