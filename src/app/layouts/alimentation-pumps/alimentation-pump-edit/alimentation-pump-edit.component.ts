...
console.log(this.alimentationPumpForm);
    ...
    if (this.alimentationPumpForm.invalid) { return; }
    this.spinner.show();
    

    this.selectedAlimentationPump.dateAlimentation = this.alimentationPumpForm.value['alimentationDate'];
    this.selectedAlimentationPump.quantity = this.alimentationPumpForm.value['quantity'];
    this.selectedAlimentationPump.fuelPump = this.alimentationPumpForm.value['fuelPump'];
    this.selectedAlimentationPump.owner=this.authentificationService.getDefaultOwner();
    console.log(this.selectedAlimentationPump);
      let res : number =0;
 let pump: Pump ;

   res =+(this.selectedAlimentationPump.quantity) as number + (this.selectFuelPump.quantity) as number;
...
this.spinner.hide();
  ...
  insertAlimentationPump(){
    ...
    this.subscriptions.add( this.alimentationPumpService.set(this.selectedAlimentationPump).subscribe(
      data => {
  ...
  onCodeFuelPumpSearch(event: any) {
    this.subscriptions.add(this.fuelPumpService.find('pump.code~' + event.query).subscribe(
      data => this.fuelPumpList = data
    ));
  }

  onReceptionSearch(event: any) {
    this.subscriptions.add(this.receptionService.find('code~' + event.query).subscribe(
      data => this.receptionList = data
    ));
  }
  ...
  onSelectReception(event) {
    ...
    this.subscriptions.add(this.receptionLineService.find('reception.code~'+ event.code).subscribe(
      data => {
        console.log(data);
